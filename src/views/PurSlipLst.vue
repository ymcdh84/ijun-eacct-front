        ...
        methods: {
            ...
            openTaxBillPopup(){
                ...
                this.$modal.open({
                    ...
                    parent: this,
                    props: {
                        issueId: this.slipList[this.rowIdx].issueId,
                        grSlipNo:this.slipList[this.rowIdx].grSlipNo,
                        vendorNm:this.slipList[this.rowIdx].payCustNm
                    },
                    width: 1200,
                    events: {
                        close(object) {
            ...
            openBulkDrfPlanPopup(){
                ...
                if(targetList.length > 0) {
                    //LIST ROW DATA 추출
                    var rowData = this.delChkList.filter(v => v.chk)

                    //팝업 param setting
                    var firstPayCustCd = rowData[0].payCustCd //payCustCd(지급거래처CD)
                    var firstWrtId = rowData[0].wrtId //wrtId(작성자ID)

                    rowData = rowData.filter(v => (v.payCustCd === firstPayCustCd) && (v.wrtId === firstWrtId))
                    ...
                    this.$modal.open({
                        ...
                        props: {
                            eaSlipNo: targetList,
                            slipTypeCd: that.form.slipTypeCd,
                            payCustCd: rowData.length > 0 ? firstPayCustCd : '',
                            wrtId: rowData.length > 0 ? firstWrtId : ''
                        },
                        parent: this,
                        width: 1200,
                        events: {
