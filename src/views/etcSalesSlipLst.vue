                    ...
                    <div class="modal-window-wrap">
                        ...
                        <div class="search_box_wrap">
                            ...
                            <div class="search_box_pop">
                                ...
                                <div class="search_con search-area">
                                    <input class="input input-bg w30p_5r" type="text" v-model="form.writerId" disabled>
        ...
        <div class="table-area">
            ...
            <div class="table-b">
                <div class="table-header">
                    ...
                    <div class="btn-wrap btn-type1 clearfix">
                        <button class="btn-size btn-gray" @click="savePayDueDt">
                            <span class="btn-icon"><i class="fas fa-file-alt"></i></span> 지급예정일 저장
                        </button>
                ...
                <div class="grid-wrap mt10">
                    <ag-grid-vue ref="grid" style="width: 100%;" class="ag-theme-alpine grid_search_height" rowSelection="multiple"
                            ...
                            @cell-value-changed="cellValueChangeList"
                            />
                </div>
            </div>
...
<script>
    ...
    //import CustomerPop from '@/components/Customer.vue'
    import CustomerPop from '@/components/Customer_Ag.vue'
    ...
    export default {
        ...
        components: {Cctr, Emp, CustomerPop, DhxGrid, DhxCalendar, AgGridVue},
        data() {
            return {
                columnDefs : [],
                gridOptions : {},
        ...
        methods: {
            ...
            cellValueChangeList(params){
              if(params.colDef.field === 'payDueDt'){
                if(!_.isEmpty(params.data.payDueDt) && params.newValue !== params.oldValue){
                  this.gridOptions.api.getRowNode(params.rowIndex).setDataValue('chk', true);
                  this.slipList[params.rowIndex].chk = true
                }
              }
            },
            calcSelAmt(){
                ...
                let totAmt = this.delChkList.reduce((prev, next) => {return prev + next.totAmt}, initValue)
            ...
            popCustomer(clear) {
                ...
                this.$modal.open({
                    ...
                    events: {
                        close(obj) {
                            vm.receiveCustomer(obj)
                        }
                    }
                })
            },
            receiveCctr(obj) {
                this.form.customerId = obj.cctrCd;
                this.form.customerNm = obj.cctrNm;
            },
            receiveEmp(obj) {
            ...
            receiveCustomer(obj) {
                ...
                this.form.customerNm = obj.customerNm;
                this.form.taxRef = obj.taxRef;
            },
            initCctr(force) {
            ...
            savePayDueDt(){
              ...
              var targetList = this.slipList.filter(v => v.chk);

              if(targetList.length > 0) {
                this.$http.post('api/erp/etc/setPayDueDt', targetList)
                    .then(response => {
                      if (!_.isEmpty(response.data)) {
                        this.$swal({type: 'success', text: '선택한 전표의 지급예정일이 정상적으로 등록되었습니다.'})
                      }
                    }).finally(() => {
                  this.goSearch()
                })
