...
<div class="inner-box">
          ...
          <tr>
            ...
            <td colspan='3'>
              <div class="datepicker w-type-ymd">
                <dhx-calendar v-model="eaSlipDt" class="input" :disabled="true"/>
              </div>
            </td>
            <th>진행상태</th>
            <td  colspan='3'>
  ...
  <!-- TABLE  -->
<div style='color:#CC3D3D'>  
※ 계약번호:&nbsp;&nbsp;접대비,&nbsp;&nbsp;회식대,&nbsp;&nbsp;교통비->9999999,&nbsp;&nbsp;현장경비->현장코드<br>
※ 계정과목:&nbsp;&nbsp;교통비->여비교통비,&nbsp;&nbsp;숙소관리비->지급수수료,&nbsp;&nbsp;소모품->소모품비,&nbsp;&nbsp;등기우편->통신비<br>
</div>
  <!-- GRID -->
  <div class="inner-box">
    <div class="table-b">
      <div class="table-header">
        ...
        <div class="btn-wrap btn-type2" v-if="slipStatCd === '10'">
          ...
          <button class="btn-size btn-excel" @click="saveExcelMst">엑셀 저장</button>
        </div>
        <div class="btn-wrap btn-type2" v-else>
          <button class="btn-size btn-excel" @click="saveExcelMst">엑셀 저장</button>
        </div>          
...
<div style='color:#CC3D3D'>
※ 증빙일자:&nbsp;&nbsp;경비청구월의 말일&nbsp;(ex:&nbsp;3월 교통비&nbsp;->&nbsp;3/31)<br>
※ 계약번호:&nbsp;&nbsp;교통비->9999999,&nbsp;&nbsp;출장비 및 현장경비-> 현장코드<br>
※ 계정과목: 교통비->여비교통비,&nbsp;&nbsp;숙소관리비->지급수수료,&nbsp;&nbsp;소모품->소모품비,&nbsp;&nbsp;등기우편->통신비<br>
</div>
    <div class="table-b">
      <div class="table-header">
        ...
        <div class="btn-wrap btn-type2" v-if="slipStatCd === '10'">
          ...
          <button class="btn-size btn-excel" @click="saveExcelSub">엑셀 저장</button>
        </div>
        <div class="btn-wrap btn-type2" v-else>
          <button class="btn-size btn-excel" @click="saveExcelSub">엑셀 저장</button>
        </div>         
          ...
          <ag-grid-vue ref="grid"
                      ...
                      :defaultColDef="defaultColDef"
                      :frameworkComponents="frameworkComponent"
                      @cell-value-changed="cellValueChange2"/>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      eaSlipDt: this.$moment().format('YYYY-MM-DD'),
  ...
  destroyed() {
    ...
    if(this.objectPopupSub.length > 0){
      for (const item of this.objectPopupSub) {
        item.close()
      }
    }
  },
  created(){
    document.title ='개인비용(경비)신청서 - IJEAS';
  ...
  methods:{
    ...
    getDeptInfo(){
      ...
      .finally(() => {
        setTimeout(() => {        
          if(this.$route.query.lnk === 'main' || this.$route.query.lnk === 'mainCard') {
    ...
    makeColumnDef(){
      ...
      this.columnDef = [
        ...
        },
        {
          ...
          field:'rmnBudgetAmt', 
          cellClass : 'bg-grey',
          cellStyle:{textAlign:'right', color:'#CC3D3D'},
          width: 130,          
          valueFormatter: (params) => { 
        ...
        {headerName:'업종명', field:'mccName', editable: false, hide: true},
      ];
      ...
      this.columnDefSub = [
        ...
        },
        {
          ...
          //   var isEa = false;
          //   if(params.data.eaSlipNo.substr(0,3) === 'TMP'){
          //     isEa = false
          //   }else{
          //     isEa = true
          //   }
          ...
          close(object) {
            if(object === undefined) {
              ...
              vm.$http.get(`/api/account/rcp`).then(response => {    
                ...
                //팝업으로 부터 넘어온 object for문
                object.forEach(async(x, i) => {
                  vm.$http.get(`/api/tmp/documentMngNo`)
                    .then(response => {
                      x.eaSlipNo = response.data
                      
                      console.log(x.apprDate)
                      
                      vm.$http.get(`/api/budget/remain`, { params: {postDt: vm.$moment(x.apprDate).format('YYYYMMDD'), cctrCd: vm.baseDeptCd} })
                      .then(response2 => {
                          
                          let remainBud = _.toInteger(response2.data);
                     
                          //업종이 불공제 업종인지 체크
                          let nonTax = false; 

                          if(vm.ndctMccList[0].remark1.indexOf(x.mccName) > -1){      
                            nonTax = true;
                          }else{
                            nonTax = acctNonTax
                          }
                          ...
                          const result = {
                            ...
                            supAmt: nonTax ? x.apprAmt + x.vat : x.apprAmt,
                            useAmt: x.purchTot,
                            vatAmt: nonTax ? 0 : x.vat,
                            ...
                            mccName: x.mccName,
                            rmnBudgetAmt: remainBud
                          }; 

                          vm.rowData.push(result);

                          vm.totAmt += Number(x.purchTot)
                      });                      
    ...
    getRemainBudget(gridDiv, rId, postDt, cctrCd) {
      this.$http.get(`/api/budget/remain`, { params: {postDt: postDt, cctrCd: cctrCd} })
        .then(response => {
            let remainBud = _.toInteger(response.data);
            
            if(gridDiv == "1"){
              this.setRemainBudget(rId, remainBud)
            }else{
              this.setRemainBudgetSubGrid(rId, remainBud)
            }
        });
    },
    setRemainBudget(rId, remainBud) {
      const rowNode = this.gridOptions.api.getRowNode(rId);
      rowNode.setDataValue('rmnBudgetAmt', remainBud);
      this.rowData[rId].rmnBudgetAmt = remainBud;
    },
    setRemainBudgetSubGrid(rId, remainBud) {
      const rowNode = this.gridOptionsSub.api.getRowNode(rId);
      rowNode.setDataValue('rmnBudgetAmt', remainBud);
      this.rowDataSub[rId].rmnBudgetAmt = remainBud;
    },
    setTotAmt(){
    ...
    buttonClickEventRemove(){
      ...
      if(selectedRows.length > 0){
        var idx = selectedRows[0].rowIndex;
      ...
      this.$modal.open({
        ...
        events: {
          close(object) {
            ...
            //잔여예산 체크
            that.getRemainBudget("1", that.rowIdx, that.rowData[that.rowIdx].eaSlipDt, that.rowData[that.rowIdx].deptCd)
            ...
            //계정 불공제 초기화(vatCd)
            rowNode.setDataValue('vatCd', "");  
            that.rowData[that.rowIdx].vatCd = "";              
    ...
    updateCctrVal(pVal){
      ...
      if(pVal){
        ...
        //잔여예산 초기화
        rowNode.setDataValue('rmnBudgetAmt', "");
        that.rowData[that.rowIdx].rmnBudgetAmt = "";
      ...
      if(pVal){
        ...
        this.$http.get('/api/account/'+ this.rowData[this.rowIdx].deptCd +'/'+this.slipTypeCd+'/'+pVal)
        .then(response => {
          ...
          if(response.data.length == 1){
            ...
            //업종 체크 후 계정 불공제 체크
            if(that.ndctMccList[0].remark1.indexOf(that.rowData[that.rowIdx].mccName) < 0){
              //불공제 계정인지 체크
              if(response.data[0].rcpYn === "Y"){
                rowNode.setDataValue('vatCd', "");
                that.rowData[that.rowIdx].vatCd = "";
              }else{
                rowNode.setDataValue('vatCd', that.cardTaxCd);
                that.rowData[that.rowIdx].vatCd = that.cardTaxCd;
              }
            }else{
              rowNode.setDataValue('vatCd', "");
              that.rowData[that.rowIdx].vatCd = "";
    ...
    cardInfoPop(idx){
      ...
      this.$modal.open({
        ...
        parent: this,
        props: {
          apprNo: this.rowData[idx].apprNo,
          crdNo: this.rowData[idx].crdNo
        },
        width: 700
      })
    },
    openAttachPopup(){
      ...
      var readonly = that.slipStatCd === '10' ? false : true;
      var chkRead = true;
      ...
      if(this.slipStatCd === '20' || this.slipStatCd === '30' || this.slipStatCd === '50'){
        ...
        */
        if(this.$store.state.loginInfo.loginId === this.wrtId){
          chkRead = false
        }
        /*
        - 재경팀 후첨 -
      ...
      let url = '/evidAtchPopModeless?docMngNo=' + that.rowData[this.rowIdx].eaSlipNo + '&readonly='+ readonly + '&readonlyCtrl='+ chkRead;

      if(!(!this.objectPopup[this.rowIdx] || this.objectPopup[this.rowIdx].closed)){
        this.objectPopup[this.rowIdx].focus();
      }else{
        this.objectPopup[this.rowIdx] = window.open(url, '_blank', 'toolbar=0,location=0,menubar=0,resizable=yes');
      }
      //팝업 Close Callback
      //브라우저 체크(IE 체크)
      ...
      if ((navigator.appName == 'Netscape' && navigator.userAgent.search('Trident') != -1) || (agent.indexOf("msie") != -1)){
        this.objectPopup[this.rowIdx].attachEvent("onunload",function(){
          rowNode.setDataValue('scanCt', localStorage.getItem("popFileCnt"));
          that.rowData[that.rowIdx].scanCt = localStorage.getItem("popFileCnt");
        });
      }else{
        this.objectPopup[this.rowIdx].onbeforeunload = function(){
          rowNode.setDataValue('scanCt', localStorage.getItem("popFileCnt"));
          that.rowData[that.rowIdx].scanCt = localStorage.getItem("popFileCnt");
        };
      }
    ...
    getNdctMccList(){
      this.$http.get('/api/code/detail', {
        params: {
          groupCd: 'NDCT_MCC_NAME'
        }
      }).then(response => {
        this.ndctMccList = response.data;
    ...
    cellValueChange2(params){
      ...
      }else if(params.colDef.field === 'eaSlipDt'){
        //잔여예산 체크
        const rowNode = params.api.getRowNode(params.node.rowIndex);
        this.getRemainBudget("2", params.node.rowIndex, rowNode.data.eaSlipDt, rowNode.data.deptCd)
      }
    },
    calcTotAmt(){
        console.log("calcTotAmt")
        let sum = 0;
    ...
    buttenClickEventCopy2(){
      ...
      if(selectedRows.length > 0){
        ...
        .then((response) => {
            ...
            copyRow.eaSlipNo = eaSlipNo

            this.rowDataSub.push(copyRow);
          
        }).catch(response => {
          console.error("buttenClickEventCopy2 Error" + response.message);
        });
    ...
    buttonClickEventAdd2(){
      ...
      .then((response) => {
        ...
        this.$http.get(`/api/budget/remain`, { params: {postDt: that.evdDt, cctrCd: that.baseDeptCd} })
        .then(response => {
            let remainBud = _.toInteger(response.data);
    ...
    buttonClickEventRemove2(){      
      var selectedRows = this.gridOptionsSub.api.getSelectedNodes() 

      if(selectedRows.length > 0){
        var idx = selectedRows[0].rowIndex
        this.rowDataSub.splice(idx, 1);
      }else{
        this.$swal({
          type: 'warning',
          text: '선택된 행이 없습니다.'
    ...
    updateCctrVal2(pVal){
      ...
      if(pVal){
        this.$http
          ...
          .then(response => {
            ...
            if(response.data.length == 1){
              ...
              //잔여예산 체크
              that.getRemainBudget("2", that.rowIdx, that.rowDataSub[that.rowIdx].eaSlipDt, that.rowDataSub[that.rowIdx].deptCd)
        ...
        //잔여 예산 초기화
        rowNode.setDataValue('rmnBudgetAmt', "");
        that.rowDataSub[that.rowIdx].rmnBudgetAmt = "";
      ...
      this.$modal.open({
        ...
        events: {
          close(object) {
            ...
            //계정 초기화
            rowNode.setDataValue('acctNm2', "");
            that.rowDataSub[that.rowIdx].acctCd = "";
            that.rowDataSub[that.rowIdx].acctNm = "";
            
            //보조계정 초기화
            rowNode.setDataValue('acctNmSub2', "");
            that.rowDataSub[that.rowIdx].acctCdSub = "";
            that.rowDataSub[that.rowIdx].acctNmSub = "";
    ...
    updateCtrlVal2(pVal){
      ...
      if(pVal){
        ...
        this.$http.get('/api/account/'+ this.rowDataSub[this.rowIdx].deptCd +'/'+this.slipTypeCd+'/'+pVal)
        .then(response => {
    ...
    openAttachPopup2(){
      ...
      var chkRead = true;
      if(this.slipStatCd === '10'){
        //작성자의 계정과 접근자의 계정이 같고 전표가 작성중 상태일때 수정 가능
        chkRead = false;
      }
      ...
      let url = '/evidAtchPopModeless?docMngNo=' + that.rowDataSub[this.rowIdx].eaSlipNo + '&readonly='+ readonly + '&readonlyCtrl='+ chkRead;

      if(!(!this.objectPopupSub[this.rowIdx] || this.objectPopupSub[this.rowIdx].closed)){
        this.objectPopupSub[this.rowIdx].focus();
      }else{
        this.objectPopupSub[this.rowIdx] = window.open(url, '_blank', 'toolbar=0,location=0,menubar=0,resizable=yes');
      }
      //팝업 Close Callback
      //브라우저 체크(IE 체크)
      var agent = navigator.userAgent.toLowerCase();
      ...
      if ((navigator.appName == 'Netscape' && navigator.userAgent.search('Trident') != -1) || (agent.indexOf("msie") != -1)){
        this.objectPopupSub[this.rowIdx].attachEvent("onunload",function(){
          rowNode.setDataValue('scanCt', localStorage.getItem("popFileCnt"));
          that.rowDataSub[that.rowIdx].scanCt = localStorage.getItem("popFileCnt");
        });
      }else{
        this.objectPopupSub[this.rowIdx].onbeforeunload = function(){
          rowNode.setDataValue('scanCt', localStorage.getItem("popFileCnt"));
          that.rowDataSub[that.rowIdx].scanCt = localStorage.getItem("popFileCnt");
        };
      }
    ...
    openUploadWingsPopup(){
      ...
      if(this.$store.state.loginInfo.loginId === this.wrtId){
        if(this.slipStatCd > '10'){
          //작성자의 계정과 접근자의 계정이 같고 전표가 작성중이 아닐때 수정 불가
          chkReadOnly = true;
        }
      }
      if(this.$store.state.loginInfo.authorities[0].roleSelectCd == '10'|| this.slipStatCd < '80'){
         //전체조회가 가능한 권한은 후첨 가능
        chkReadOnly = false;
      }
      ...
      this.$modal.open({
        ...
        props: {
          grSlipNo: copy ? this.tmpSlipNo : this.grSlipNo || this.tmpSlipNo,
    ...
    getDetail(){
      ...
      .then(response => {
        ...
        this.tmpSlipNo = data_gr.grSlipNo;
      ...
      .finally(() => {
        if(this.$route.params.copy){
          this.$http.get(`/api/wings/document/${this.tmpSlipNo}`).then((response) => {
            if(response.data) {
              this.tempData = response.data
              this.tempData.forEach(x => {
                x.grSlipNo = this.tmpSlipNo
                delete x.documentId
              })
            }
          })
    ...
    saveExcelSub() {
        ...
        this.gridOptionsSub.api.exportDataAsCsv(params);   
    }
  },
  watch: {
    'slipStatCd': {
