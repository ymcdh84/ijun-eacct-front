<template>
  <div class="modal-card pop_min3" id="draggable">
...
export default {
  ...
  methods: {
    move() {
      if(this.isMxMn) {
        $(".modal-card").removeClass('pop_min3')
        $(".modal-card").addClass('pop_max');

        $(".animation-content").removeClass('pop_min3')
        $(".animation-content").addClass('pop_max');
        this.isMxMn = false
      } else {
        $(".modal-card").removeClass('pop_max')
        $(".modal-card").addClass('pop_min3');

        $(".animation-content").removeClass('pop_max')
        $(".animation-content").addClass('pop_min3');
        this.isMxMn = true
      }
    }
  },
...
<style>
.modal-card-head {background: #0065b3;}
.modal-card-title {color:white; font-size:1.2em;}
.modal-close {top: 80px;}
