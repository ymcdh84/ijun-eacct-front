<template>
    <div>
        <div class="table-area" style="margin-top: 0px;" v-show="chkWeb">
            <appr-header :doc-type="docType" :apprHeader="apprHeader" :appr-line="apprLine" v-model="data"/>
        </div>
        <!-- //Appr Header area -->
        <component :is="docComponent" v-model="data" :budReqNo="budReqNo" :readonly="true"/>
    </div>
</template>
...
<script>
    ...
    export default {
                ...
                if(docMngNo !== null) {
                    ...
                    .then(response => {
                        if (response.data) {
                            ...
                            }else if(response.data.budgetHeader) {
                              this.data = response.data.budgetHeader
                              this.data.budgetDetails = response.data.budgetDetails
                              this.apprHeader = response.data.apprHeader[0];
                              this.apprLine = response.data.apprDetails;
        ...
        created() {
            ...
            // if (this.value.elecApprId !== undefined) this.getAppr(this.value.elecApprId)
            // if (this.docMngNo === undefined && this.budReqNo === undefined){
            if (this.docMngNo === undefined){
              this.queryApprLineRule();
            }

            if(this.$route.path.substr(0,7)=='/mobile') {this.chkWeb = false}
        },
        mounted() {
            console.log('ApprDtl1 : '+this.docMngNo)
            console.log('ApprDtl2 : '+this.budReqNo)
            // console.log('ApprDtl3 : ', this.value.elecApprId)
            this.$store.commit('finish')
        },
        watch: {
            value: {
