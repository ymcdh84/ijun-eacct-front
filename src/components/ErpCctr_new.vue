...
export default {
    ...
    return {
      ...
      grid: {
        head: [
          ...
          {id: 'deptCd', type: 'ro', align: 'center', sort: 'str', value: '비용부서코드'},
          {id: 'deptNm', type: 'ro', align: 'left', sort: 'str', value: '비용부서명'},
          {id: 'expenseFlag', type: 'ro', align: 'center', sort: 'str', value: '비용구분'},
        ],
        data: [],
      },
      returnObject: {}
  ...
  methods: {
    ...
    goSearch(slow) {
      ...
      this.grid.data = [];
      this.$http.get(`/api/cctr/erp/${this.search}`)
        .then((result) => {
    ...
    doReturnObject(rId) {
      ...
      this.returnObject.deptCd = this.$refs.grid.cells(rId, 1).getValue();
      this.returnObject.deptNm = this.$refs.grid.cells(rId, 2).getValue();
